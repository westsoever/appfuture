...
## Step 1 — Load context
Read `ACTIVE_PLAN.md` in full. Read `CLAUDE.md` for module map.
...
Cross-reference the results against the Tasks list in `ACTIVE_PLAN.md`:
...
- Update `ACTIVE_PLAN.md` with any changes.

## Step 4 — Pick the next task
Take the first unchecked item in "Up Next". Add it to "In Progress" in `ACTIVE_PLAN.md`. Say out loud which task you're starting.
...
- When a task is fully done, move it from "In Progress" to "Done" in `ACTIVE_PLAN.md`

## Step 6 — After each task completes
Update `ACTIVE_PLAN.md` before moving to the next task. Then loop back to Step 4.
